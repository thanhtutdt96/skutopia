...
export const loadFixture = <T>(path: string): T => JSON.parse(fs.readFileSync(`exercise/${path}`, 'utf8'));

export const calculateCarrierFees = (carrier: Carrier, items: SalesOrder['items']): number => {
  switch (carrier) {
    case 'UPS':
...
export const generateQuote = (order: SalesOrder, carrier: Carrier): ShippingQuote => ({
    carrier,
